  ...
  if (step === 'loading') return (
    <div className="p-8 flex flex-col items-center justify-center min-h-[70vh]">
      <div className="relative mb-6">
        <div className="h-20 w-20 border-4 border-teal-100 border-t-teal-600 rounded-full animate-spin" />
      ...
      <div className="p-8 max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-2 mb-8">
          {STEPS.map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`h-6 w-6 rounded-full flex items-center justify-center text-xs font-bold ${i <= 1 ? 'bg-teal-600 text-white' : 'bg-slate-200 text-slate-400'}`}>{i + 1}</div>
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="col-span-2 space-y-4">
            {/* Urgency */}
            <div className={`border rounded-2xl px-5 py-4 ${urg.bg} ${urg.border}`}>
              ...
              {conditions.length === 0 ? (
                ...
                <div className="space-y-3">
                  {conditions.map((c, i) => {
                    ...
                    return (
                      <div key={i} className={`border rounded-xl p-4 ${cls.wrap}`}>
                        <div className="flex items-start justify-between gap-3 mb-2">
                          ...
                          <div className="flex-shrink-0 text-right">
                            ...
                            <div className="h-1.5 w-20 bg-slate-200 rounded-full mt-1 overflow-hidden">
                              <div className={`h-full rounded-full ${cls.bar}`} style={{ width: `${c.confidence}%` }} />
                            </div>
                          </div>
                        </div>
                        ...
                        {c.matchedSymptoms.length > 0 && (
                          <div className="flex flex-wrap gap-1">
                            {c.matchedSymptoms.map(s => (
                              <span key={s} className="text-xs bg-white/70 border border-slate-200 rounded-full px-2 py-0.5 text-slate-600">{s}</span>
                            ))}
                          </div>
                        )}
                      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl p-5 text-white" style={{ background: 'linear-gradient(135deg,#0d9488,#0f766e)' }}>
              ...
              <button onClick={() => router.push(`/patient/appointments?specialty=${encodeURIComponent(recommendedSpecialty)}&sessionId=${session.id}`)}
                className="block w-full text-center bg-white text-teal-700 font-bold px-4 py-2.5 rounded-xl text-sm hover:bg-teal-50 transition-colors">
                Book appointment →
              </button>
            </div>
          </div>
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="flex items-center gap-2 mb-8">
        {STEPS.map((s, i) => (
          <div key={s} className="flex items-center gap-2">
            <div className={`h-6 w-6 rounded-full flex items-center justify-center text-xs font-bold ${i === 0 ? 'bg-teal-600 text-white' : 'bg-slate-200 text-slate-400'}`}>{i + 1}</div>
            ...
            <div className="flex flex-wrap gap-2">
              {COMMON_SYMPTOMS.map(s => (
                <button key={s} onClick={() => toggleSymptom(s)}
                  className={`px-3 py-1.5 rounded-full text-sm border font-medium transition-all duration-150 ${
                    selected.includes(s)
          ...
          <ol className="space-y-3">
            {[
              ...
              <li key={i} className="flex items-start gap-2.5">
                <span className="h-5 w-5 rounded-full bg-teal-100 text-teal-700 text-xs flex items-center justify-center font-bold flex-shrink-0 mt-0.5">{i+1}</span>
